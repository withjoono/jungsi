...
const ExploreEarlySubjectStepperContext = createContext<
  StepperContextProps | undefined
>(undefined);

const ExploreEarlySubjectStepperProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  return (
    <ExploreEarlySubjectStepperContext.Provider value={value}>
      {children}
    </ExploreEarlySubjectStepperContext.Provider>
  );
};

const useExploreEarlySubjectStepper = (): StepperContextProps => {
  const context = useContext(ExploreEarlySubjectStepperContext);
  if (!context) {
    throw new Error(
      "useExploreEarlySubjectStepper must be used within a ExploreEarlySubjectStepperProvider",
    );
  }
  return context;
};

export { ExploreEarlySubjectStepperProvider, useExploreEarlySubjectStepper };
