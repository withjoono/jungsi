...
import { EarlySubjectStep3GroupData } from "./step-3";

interface GroupedDataTableProps {
  className?: string;
  groupedData: {
    [key: string]: EarlySubjectStep3GroupData;
  };
...
export const GroupedDataTable = ({
  ...
  const onClick = (item: EarlySubjectStep3GroupData) => {
  ...
  const checkSelectedItem = (item: EarlySubjectStep3GroupData) => {
    const key = `${item.university_name}-${item.university_region}-${item.admission_name}-${item.general_field_name}`;
