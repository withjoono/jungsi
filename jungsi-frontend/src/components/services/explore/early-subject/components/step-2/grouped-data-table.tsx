...
import { IEarlySubjectStep2GroupData } from "./step-2";

interface GroupedDataTableProps {
  className?: string;
  groupedData: {
    [key: string]: IEarlySubjectStep2GroupData;
  };
...
export const GroupedDataTable = ({
  ...
  const onClick = (item: IEarlySubjectStep2GroupData) => {
  ...
  const checkSelectedItem = (item: IEarlySubjectStep2GroupData) => {
    const key = `${item.university_name}-${item.university_region}-${item.admission_name}-${item.general_field}-${item.description}`;
