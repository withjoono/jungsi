  ...
  const gradeData = useMemo(() => {
    ...
    subjects.forEach((subject) => {
      if (
        ...
        if (
          data[key] &&
          Object.values(SUBJECT_CODES).includes(subject.main_subject_code)
        ) {
          if (!data[key][subject.main_subject_code]) {
            data[key][subject.main_subject_code] = { grades: [], units: [] };
          }
          data[key][subject.main_subject_code].grades.push(
            parseFloat(subject.ranking),
          );
          data[key][subject.main_subject_code].units.push(
