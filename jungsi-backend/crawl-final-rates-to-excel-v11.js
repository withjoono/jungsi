/**
 * 모든 대학 최종 경쟁률 엑셀 크롤링 스크립트 v11
 *
 * v11 개선사항:
 * - 동명대학교, 신라대학교 전용 파서 추가
 *   - 요약 테이블(첫 헤더="구분") 스킵
 *   - 상세 테이블(첫 헤더="대학") 파싱
 * - rowspan 파서: 숭실대, 경성대, 한국체육대에만 적용
 *   (동의대, 서원대, 우송대, 대구가톨릭대, 동서대는 표준 파서가 더 나은 결과를 보임)
 *
 * 사용법: node crawl-final-rates-to-excel-v11.js
 */
...
const SPECIAL_PARSER_UNIVS = [
  ...
  '경성대학교',
  '한국체육대학교',
  // v11 추가: 동명대, 신라대 (요약/상세 테이블 분리 구조)
  '동명대학교',
  '신라대학교',
  // Note: 동의대, 서원대, 우송대, 대구가톨릭대, 동서대는 rowspan 파서 적용 시 오히려 악화됨
  // 표준 파서로 처리하는 것이 더 나은 결과를 보임
];
...
// ==================== rowspan 처리 파서 (숭실대/경성대/한국체육대/동의대/서원대/우송대/대구가톨릭대/동서대) ====================
...
// ==================== 동명대학교 전용 파서 (v11 신규) ====================
// 동명대 구조:
// - 요약 테이블: 구분 | 전형유형 | 전형명 | 모집인원 | 지원인원 | 경쟁률 (스킵)
// - 상세 테이블: 대학 | 모집단위(전공명) | 슬로건(특별장학) | 모집인원 | 지원인원 | 경쟁률 | 학과홈페이지
//   → 대학(0), 모집단위(1), 슬로건(2), 모집인원(3), 지원인원(4), 경쟁률(5)
...
function parseDongmyungTable($, table, universityName, defaultGroup, defaultAdmissionType) {
  ...
  // 요약 테이블 스킵: 첫 번째 헤더가 "구분"인 경우
  if (headerTexts.length > 0 && headerTexts[0] === '구분') {
    return { results: [], errors: [] };
  }

  // 상세 테이블: 첫 번째 헤더가 "대학"인 경우만 처리
  if (headerTexts.length === 0 || headerTexts[0] !== '대학') {
    return { results: [], errors: [] };
  }

  // 동명대 상세 테이블 컬럼 구조 (7컬럼):
  // 대학(0) | 모집단위(1) | 슬로건(2) | 모집인원(3) | 지원인원(4) | 경쟁률(5) | 학과홈페이지(6)
  const COL = {
    college: 0,
    department: 1,
    slogan: 2,
...
// ==================== 신라대학교 전용 파서 (v11 신규) ====================
// 신라대 구조:
// - 요약 테이블: 구분 | 전형명 | (군별) | 모집인원 | 지원인원 | 경쟁률 (스킵)
// - 상세 테이블: 대학 | 모집단위 | 모집군 | 국고지원사업참여현황 | 모집인원 | 지원인원 | 경쟁률 | ...
//   → 대학(0), 모집단위(1), 모집군(2), 국고지원(3), 모집인원(4), 지원인원(5), 경쟁률(6)
...
function parseSillaTable($, table, universityName, defaultGroup, defaultAdmissionType) {
  ...
  // 신라대 상세 테이블 컬럼 구조 (10컬럼):
  // 대학(0) | 모집단위(1) | 모집군(2) | 국고지원사업(3) | 모집인원(4) | 지원인원(5) | 경쟁률(6) | 전공가이드북(7) | 취업성공로드맵(8) | 학과소개영상(9)
  const COL = {
    college: 0,
    department: 1,
    group: 2,
  ...
  try {
    ...
    $('table').each((tableIdx, table) => {
      ...
      if (universityName === '연세대학교(서울)') {
        ...
        '숭실대학교', '경성대학교', '한국체육대학교'
        // Note: 동의대, 서원대, 우송대, 대구가톨릭대, 동서대는 rowspan 파서 적용 시 오히려 악화됨
        // 표준 파서가 더 나은 결과를 보임 (v11에서 제거)
      ].includes(universityName)) {
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║   정시 경쟁률 크롤러 v11 - 동명대/신라대 전용 파서 추가      ║');
  console.log('╠══════════════════════════════════════════════════════════════╣');
  ...
  saveToExcel(allData, path.join(outputDir, `최종경쟁률_v11_${dateStr}_${timeStr}.xlsx`));
  ...
  saveToExcel(summaryData, path.join(outputDir, `최종경쟁률_요약_v11_${dateStr}_${timeStr}.xlsx`), '대학별요약');

  if (CONFIG.LOG_ERRORS_TO_FILE && allErrors.length > 0) {
    saveErrorLog(allErrors, path.join(outputDir, `크롤링에러_v11_${dateStr}_${timeStr}.xlsx`));
  }
