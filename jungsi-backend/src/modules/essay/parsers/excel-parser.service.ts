...
import {
  ParsedEssay,
  ParsedEssayLowestGrade,
  ParsedEssayAvailableMath,
  ParsedEssayPercentile,
  ParsedEssayResult,
} from './essay-parser.types';
...
export class EssayExcelParserService {
   ...
   */
  parseEssayExcel(buffer: Buffer, columnIndex: number): ParsedEssayResult {
    this.logger.log('Essay Excel 데이터 파싱 시작');

    const essays: ParsedEssay[] = [];
    const lowestGrades: ParsedEssayLowestGrade[] = [];
    ...
    for (let i = 0; i < dataArray.length; i++) {
      ...
      try {
        ...
        const lowestGrade: ParsedEssayLowestGrade = {
          collegeCode: row['대학교코드'] || '',
          essayId: id,
    ...
    this.logger.log(`파싱 완료: ${essays.length}개 essays, ${lowestGrades.length}개 lowest grades`);
    return { essays, lowestGrades };
  }
  ...
  parseEssayAvailableMathExcel(
    ...
    const result: ParsedEssayAvailableMath[] = [];
    ...
    for (const row of dataArray) {
      const item: ParsedEssayAvailableMath = {
  ...
  parseEssayPercentileExcel(
    ...
    const result: ParsedEssayPercentile[] = [];
    const data = ExcelUtil.getRowDataExceptTopColumn(buffer, columnIndex);
    const dataArray = ExcelUtil.mapArrayToObjectArray(data);

    for (const row of dataArray) {
      const item: ParsedEssayPercentile = {
