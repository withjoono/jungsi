...
export class SchoolRecordController {
  constructor(
    ...
    private readonly pdfParserService: SchoolRecordPdfParserService,
  ) {}
   ...
   */
  @Post('parse/pdf')
  @ApiOperation({ summary: 'PDF 학생부 파싱 (졸업생용)' })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
      type: 'object',
  ...
  async parsePdfSchoolRecord(
    ...
    const result = await this.pdfParserService.parse(file.buffer);
